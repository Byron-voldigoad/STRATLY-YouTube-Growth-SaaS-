...
export class AiInsightsComponent implements OnInit {
  ...
  async loadStoredAnalyses() {
    try {
      ...
      // 1. Récupérer l'audit (channel)
      const channelResponse = await this.genkit.getLatestAnalysis(profile.id, profile.youtube_channel_id, 'channel');
      if (channelResponse?.result) {
        const parsed = channelResponse.result;
        if (parsed.channelStatus && parsed.recommendation?.action) {
          parsed.patterns = parsed.patterns || { toAvoid: [], toRepeat: [] };
          parsed.patterns.toAvoid = Array.isArray(parsed.patterns.toAvoid) ? parsed.patterns.toAvoid : [];
          parsed.patterns.toRepeat = Array.isArray(parsed.patterns.toRepeat) ? parsed.patterns.toRepeat : [];
      ...
      const ideasResponse = await this.genkit.getLatestAnalysis(profile.id, profile.youtube_channel_id, 'ideas');
      if (ideasResponse?.result && Array.isArray(ideasResponse.result)) {
         this.ideas = ideasResponse.result;
         console.log('Loaded stored ideas:', this.ideas.length);
      }

      // 3. Récupérer lastVideoDaysAgo pour l'UI
      ...
      if (videos && videos.length > 0) {
        ...
        this.lastVideoDaysAgo = Math.floor(
          (Date.now() - new Date(sorted[0].published_at).getTime()) / (1000 * 60 * 60 * 24)
        );
      }
  ...
  /**
   * Navigue vers la page décision en transmettant les insights de l'audit
   * via le state du router (chaînage Audit → Decide)
   */
  goToDecision() {
    if (!this.analysisResult) return;

    const auditInsights = {
      channelStatus: this.analysisResult.channelStatus,
      statusExplanation: this.analysisResult.statusExplanation,
      engagement: this.analysisResult.metrics.engagement,
      trend: this.analysisResult.metrics.trend,
      patternsToRepeat: this.analysisResult.patterns.toRepeat,
      patternsToAvoid: this.analysisResult.patterns.toAvoid,
      recommendedAction: this.analysisResult.recommendation.action,
      recommendedProof: this.analysisResult.recommendation.proof,
      recommendedNextStep: this.analysisResult.recommendation.nextStep,
    };
