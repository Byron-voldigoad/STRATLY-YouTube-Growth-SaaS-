...
export class GenkitService {
  ...
  /**
   * Récupère la dernière analyse stockée en base de données.
   */
  async getLatestAnalysis(
    ...
    try {
      const response = await lastValueFrom(
        this.http.get<{ success: boolean; result: any; updatedAt: string }>(
          `${this.apiUrl}/analyses/latest`,
          { params: { userId, channelId, type } }
        )
      );
      if (response.success && response.result) {
        return { result: response.result, cached: true };
      }
      return null;
    } catch (error) {
