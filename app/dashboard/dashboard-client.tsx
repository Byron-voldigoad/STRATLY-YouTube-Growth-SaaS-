...
interface DashboardClientProps {
    initialChannel: ChannelStats | null;
    initialAnalyticsData: AnalyticsData[];
    initialVideos: VideoData[];
    userId: string;
}

export function DashboardClient({ initialChannel, initialAnalyticsData, initialVideos, userId }: DashboardClientProps) {
  const router = useRouter();
  const supabase = createClient();

  const [channel, setChannel] = useState<ChannelStats | null>(initialChannel);
  ...
  // This effect is to re-fetch data when the time range changes.
  ...
  function getBestPerformingDay(videosList: VideoData[]): string {
    ...
    const dayPerformance: Record<
      number,
      { videos: number; totalViews: number; avgViews: number }
    > = {};
    ...
    Object.keys(dayPerformance).forEach((dayStr) => {
      ...
      if (dayPerformance[day].videos > 0) {
        dayPerformance[day].avgViews =
          dayPerformance[day].totalViews / dayPerformance[day].videos;
      }
    });
    ...
    Object.keys(dayPerformance).forEach((dayStr) => {
      const day = parseInt(dayStr);
      if (
        dayPerformance[day]?.videos > 0 &&
        dayPerformance[day].avgViews > bestAvgViews
      ) {
    ...
    const context =
      totalVideosOnBestDay > 1
        ? ` (${totalVideosOnBestDay} vidéos, ${Math.round(
            bestAvgViews
          )} vues moy.)`
    ...
    Object.keys(slotPerformance).forEach((slotName) => {
      if (slotPerformance[slotName].videos > 0) {
        slotPerformance[slotName].avgViews =
          slotPerformance[slotName].totalViews /
          slotPerformance[slotName].videos;
      }
    });
  ...
  function getAveragePublishingTime(videosList: VideoData[]): string {
    ...
    const hours = videosList.map((video) =>
      new Date(video.published_at).getHours()
    );
    const minutes = videosList.map((video) =>
      new Date(video.published_at).getMinutes()
    );
    ...
    return `${avgHour.toString().padStart(2, "0")}h${avgMinute
      .toString()
      .padStart(2, "0")}`;
  }
  ...
  function getBestPerformingMonth(videosList: VideoData[]): string {
    ...
    Object.keys(monthPerformance).forEach((monthStr) => {
      ...
      if (monthPerformance[month].videos > 0) {
        monthPerformance[month].avgViews =
          monthPerformance[month].totalViews / monthPerformance[month].videos;
      }
    });
    ...
    Object.keys(monthPerformance).forEach((monthStr) => {
      const month = parseInt(monthStr);
      if (
        monthPerformance[month]?.videos > 0 &&
        monthPerformance[month].avgViews > bestAvgViews
      ) {
    ...
    const context =
      monthPerformance[bestMonth]?.videos > 1
        ? ` (${monthPerformance[bestMonth].videos} vidéos, ${Math.round(
            bestAvgViews
          )} vues moy.)`
        : ` (${Math.round(bestAvgViews)} vues)`;
  ...
  async function handleImportData() {
    try {
      ...
      if (response.status === 202) {
        alert("L'importation des données a commencé. Cela peut prendre quelques minutes. Vous pouvez continuer à naviguer.");
        // router.refresh() is removed as the process is now in the background
      } else if (response.ok) {
        alert("L'opération s'est terminée rapidement !");
        router.refresh(); 
      } else {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
            ...
            <div className="mt-2 text-sm text-green-500">+0% ce mois-ci</div>
          </div>
              ...
              {videos.length > 0 ? (
                      ...
                      <div className="space-y-3">
                        <div>
                          ...
                          <div className="font-semibold text-white text-lg">
                            {Math.round(
                              calculateTotalViews(videos) / videos.length
                            )}
                          </div>
                        </div>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-500 mb-4">
                    Importez vos données YouTube pour voir des insights
                    personnalisés
                  </p>
                  <button
                    onClick={handleImportData}
