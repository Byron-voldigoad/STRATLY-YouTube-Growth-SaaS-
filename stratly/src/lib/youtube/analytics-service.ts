  ...
  private async getAuthenticatedClient(userId: string) {
    ...
    if (isTokenExpired) {
      ...
      try {
        ...
        // ⭐ GESTION DE L'ERREUR invalid_grant
        if (error.message && (error.message.includes('invalid_grant') || error.code === '400' || error.response?.data?.error === 'invalid_grant')) {
          console.log(`🔴 Token invalide pour l'utilisateur ${userId}. Nettoyage...`);
          ...
          // Lancer une erreur spécifique pour le frontend
          throw new Error('REAUTH_REQUIRED: Votre connexion YouTube a expiré. Veuillez vous reconnecter.');
        }
