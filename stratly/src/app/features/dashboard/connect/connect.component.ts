...
import { lucideYoutube, lucideCheckCircle2, lucideAlertCircle, lucideExternalLink } from '@ng-icons/lucide';
...
@Component({
  ...
  providers: [
    provideIcons({ lucideYoutube, lucideCheckCircle2, lucideAlertCircle, lucideExternalLink })
  ],
  template: `
    <div class="space-y-8 animate-in fade-in duration-500">
      <div class="flex flex-col gap-2">
        <h2 class="text-3xl font-bold tracking-tight text-slate-900 font-heading">Connexion YouTube</h2>
        <p class="text-muted-foreground">Gérez la connexion entre Stratly et votre chaîne YouTube.</p>
      </div>
          ...
          <div hlmCardContent class="py-6">
            @if (isLoading) {
              ...
              <div class="flex flex-col gap-6">
                ...
                @if (isConnected) {
                  <div class="p-4 bg-slate-50 rounded-xl border border-border/10 space-y-3">
                    <p class="text-sm text-slate-600 leading-relaxed">
                      Votre chaîne est correctement liée. Stratly importe automatiquement vos données pour générer des analyses.
                    </p>
                    <div class="flex gap-2">
                       <button hlmBtn variant="outline" size="sm" class="gap-2 text-xs" (click)="connect()">
                  ...
                  <div class="space-y-4">
                    <p class="text-sm text-muted-foreground">
                      Connectez votre compte Google pour autoriser Stratly à accéder à vos statistiques YouTube Analytics.
                    </p>
